...
try:
    # older/newer Qiskit variants
    from qiskit.quantum_info import PauliTransferMatrix as PTMClass
except ImportError:
    from qiskit.quantum_info import PTM as PTMClass
...
def projector_traceless(n_qubits: int) -> np.ndarray:
    """
    P_ad^(n): projector onto the traceless Pauli subspace
    in the Pauli-transfer-matrix basis.
    """
    dim = 4**n_qubits
    P = np.eye(dim)
    P[0, 0] = 0.0  # kill the identity component
    return P


def probe_A_from_unitary(qc) -> np.ndarray:
    """
    Given a QuantumCircuit 'qc' implementing a unitary U on n qubits,
    return A = P_ad R(U) P_ad in the PTM basis.
    """
    n_qubits = qc.num_qubits
    R = PTMClass(Operator(qc)).data   # (4^n x 4^n) real matrix
    P = projector_traceless(n_qubits)
    return P @ R @ P
